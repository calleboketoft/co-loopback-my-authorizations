...
module.exports = function (User) {
    ...
    User.myAuthorizations = function (cb) {
        ...
        RoleMapping.find({ where: { principalType: 'USER', principalId: userId } }, function (err, roleMappings) {
            debug('found %s', roleMappings);
            var rolesToFetch = roleMappings.map(function (roleMapping) {
                return {
                    id: roleMapping.roleId
                };
            });
            Role.find({ where: { or: rolesToFetch } }, function (err, roles) {
                var aclsToFetch = roles.map(function (role) {
                ...
                ACL.find({ where: { or: aclsToFetch } }, function (err, acls) {
                    authorizations.acls = acls.map(function (acl) {
                        return {
                            model: acl.model,
